...
CHART TYPE NAME MAPPING (must match visualizationsMaster.js exactly):
...
def _chart_histogram(df, col):
    ...
    if len(clean) >= 3:
        ...
        fig.add_trace(go.Scatter(x=kde_x, y=kde(kde_x) * scale, mode='lines', name='KDE',
...
def _chart_qq(df, col):
    ...
    fig.update_layout(_layout(title=f'📐 {CHART_LABELS["qq"]}: {col}',
...
def _chart_violin(df, col):
    fig = go.Figure(go.Violin(y=df[col].dropna(), name=col,
...
def _chart_scatter(df, col_x, col_y):
    fig = px.scatter(df, x=col_x, y=col_y, opacity=0.65,
...
def _chart_scatter_matrix(df, num_cols):
    ...
    if df_s.empty:
        return None
    fig = px.scatter_matrix(df_s, dimensions=cols,
...
def _chart_regression_plot(df, col_x, col_y):
    # Prevent 500s for invalid selections
    if col_x == col_y:
        # fallback to a safe scatter plot
    ...
    # If x has zero variance, linregress may fail; fallback to scatter
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_r[col_x], y=df_r[col_y], mode='markers',
                             name='Data', marker=dict(color=PALETTE[0], size=6, opacity=0.6),
    ...
    fig.update_layout(_layout(title=f'📈 {CHART_LABELS["regression_plot"]}: {col_x} → {col_y} | R²={r**2:.3f}',
...
def _chart_bubble_chart(df, col_x, col_y, col_z):
    ...
    if size_range == 0:
        size_scaled = pd.Series([20] * len(df_b))
    else:
        size_scaled = (df_b[col_z] - df_b[col_z].min()) / size_range * 35 + 5
    fig = go.Figure(go.Scatter(
        x=df_b[col_x], y=df_b[col_y], mode='markers',
        marker=dict(size=size_scaled, color=PALETTE[0], opacity=0.65,
                    line=dict(width=0.5, color='white'),
                    sizemode='diameter'),
        hovertemplate=f'{col_x}: %{{x:.2f}}<br>{col_y}: %{{y:.2f}}<br>{col_z}: %{{marker.size:.1f}}<extra></extra>',
    ))
    fig.update_layout(_layout(title=f'🫧 {CHART_LABELS["bubble_chart"]}: {col_x} × {col_y} (size={col_z})',
                              xaxis_title=col_x, yaxis_title=col_y))
    return _json(_axes(fig))
...
def _chart_box_cat_num(df, cat_col, num_col):
    ...
    fig = px.box(sub, x=cat_col, y=num_col, color=cat_col,
                 color_discrete_sequence=PALETTE,
...
def _chart_violin_cat_num(df, cat_col, num_col):
    top = df[cat_col].value_counts().head(8).index
    sub = df[df[cat_col].isin(top)]
    fig = px.violin(sub, x=cat_col, y=num_col, color=cat_col,
...
def _chart_grouped_bar(df, cat_col, num_col):
    ...
    fig = go.Figure(go.Bar(x=grp.index.astype(str), y=grp.values,
...
def _chart_strip_plot(df, cat_col, num_col):
    ...
    sub = df[df[cat_col].isin(top)]
    fig = px.strip(sub, x=cat_col, y=num_col,
                   color_discrete_sequence=[PALETTE[0]],
...
def _chart_grouped_bar_compare(df, num_cols):
    means = [float(df[c].mean()) for c in num_cols[:10]]
    stds  = [float(df[c].std())  for c in num_cols[:10]]
    fig = go.Figure(go.Bar(
        x=num_cols[:10], y=means,
...
def _chart_parallel_coords(df, num_cols):
    ...
    dims = [dict(label=c, values=sub[c]) for c in cols]
    color_vals = np.arange(len(sub))
...
def generate_master_chart(df, num_cols, cat_cols, category, chart_type,
    ...
    """
    Generate single chart + KPI metadata for the master visualization view.
    Returns dict: ok, chart, kpis, chart_label, chart_index, chart_total, ...
    """
    ...
    try:
        builders = {
            ...
            # categorical charts (bar/pie/count/pareto) are only valid if col_x exists in df.
            # (kept here intentionally to avoid accidental None due to wrong defaults)
